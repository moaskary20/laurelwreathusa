<x-filament-panels::page>
    <div
        class="ci-wajebaty ci-payroll-wajebaty ci-reports-page ci-vat-customers-report-page -mx-4 -mt-8 px-4 pb-10 pt-2 sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8"
        dir="rtl"
        wire:key="vat-customers-report-{{ \Filament\Facades\Filament::getTenant()?->getKey() }}"
    >
        ...
        <div class="ci-rep-title">ضريبة القيمة المضافة للعملاء</div>

        <div class="ci-card ci-form-inner ci-reports-card ci-rep-no-print">
            <div class="ci-rep-filters">
                <div class="ci-rep-field">
                    <label for="vat-cust-select">اسم العميل</label>
                    <select id="vat-cust-select" wire:model.live="customerId">
                        <option value="">اختيار عميل — الكل</option>
                ...
                <div class="ci-rep-field">
                    <label for="vat-cust-from">التاريخ من:</label>
                    <input id="vat-cust-from" type="date" wire:model="dateFrom" />
                </div>
                <div class="ci-rep-field">
                    <label for="vat-cust-to">إلى:</label>
                    <input id="vat-cust-to" type="date" wire:model="dateTo" />
                </div>
                <x-filament::button
                    type="button"
                    wire:click="search"
        ...
        <div class="ci-card ci-form-inner ci-reports-card">
            ...
            <div class="ci-rep-meta">
                <div><span>اسم العميل :</span> {{ $this->headerCustomerLabel() }}</div>
                ...
                <table>
                    ...
                    <tbody>
                        @if (! $hasSearched)
                            <tr>
                                <td
                                    colspan="{{ ($customerId === '' || $customerId === null) ? 6 : 5 }}"
                                    class="ci-rep-empty"
                                >
